window.onload = function(){
  const backgroundPopup = document.querySelector('.background'),
      ...
      basketSlogan = conteinerOrder.querySelector('.slogan-add'),
      orderTotal = conteinerOrder.querySelector('.order-totals');
...
function activCart(num) {
 const item = templateProduct.cloneNode(true),
 ...
 function creatItem (object, weight) {
  const element = templateOrder.cloneNode(true),
 ...
 function closeCart(item) {
  const closeCart = item.querySelector('.close-up');
...
function OpenBasket () {
  conteinerBasket.addEventListener('mouseenter', function() {
    ...
    conteinerOrder.classList.add('order-show');   
  })
    conteinerBasket.addEventListener('mouseleave', function() {
